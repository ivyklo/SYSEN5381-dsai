...
DOCUMENT = "data/plans/long_beach_nyrcr_plan.txt"  # path to text doc
...
def parse_llm_json(text: str):
    """JSON from chat models: strip optional ``` fences; strict=False allows newlines inside strings."""
    text = text.strip()
    if text.startswith("```"):
...
# A real query from a user! (Long Beach NYRCR Plan: coastal flood/surge, boardwalk, dunes, stormwater, implementation projects)
query = (
    "How does the Long Beach NY Rising Community Reconstruction Plan address coastal flood and storm surge risk "
    "after Superstorm Sandy—including shoreline, dunes, and boardwalk-related strategies—and how do those "
    "connect to the community's broader recovery and resilience priorities?"
)
...
role = (
    "You are a helpful assistant that answers questions using only the Long Beach, New York "
    "NY Rising Community Reconstruction (NYRCR) Plan (March 2014): a post–Superstorm Sandy community plan "
    "covering flood risk, storm surge, infrastructure, the shoreline and boardwalk area, and implementation projects. "
    "Answer the question provided by the user, using only the context provided by the user. "
    "If the context is insufficient, say what is missing rather than inventing details. "
    "Format your response as markdown with a title and clear bullet points. "
    "Content is provided as: <user original query> | <context from vector database search>"
)
...
print(result3)
try:
    print(parse_llm_json(result3))
except json.JSONDecodeError as e:
    print("Could not parse fact-check output as JSON:", e)
    print("Raw:", result3)
